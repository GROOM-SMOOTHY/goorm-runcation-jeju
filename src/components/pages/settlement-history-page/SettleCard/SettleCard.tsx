...
const SettleCard: React.FC<SettleCardProps> = ({
  ...
  const handleStatusToggle = async (checked: boolean) => {
    try {
      ...
      addToast("오류 발생", err.message, "error");
      // 에러 발생 시 UI 상태 롤백
  ...
  const isProgressFull =
    totalMemberCount > 0 && completedCount === totalMemberCount;
            ...
            <div className={styles.section}>
              ...
              <div className={styles.memberTags}>
                {pendingSorted.map((m) => (
                  <div
                    key={m.name}
                    className={`${styles.memberTag} ${currentUserName != null && m.name === currentUserName ? styles.memberTagIsMe : ""}`}
                  >
